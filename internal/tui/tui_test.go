...
func TestFormatToolCardExpanded(t *testing.T) {
	...
	out := formatToolCard(c)
	// Check for capitalized labels in new pretty format
	if !strings.Contains(out, "Input:") && !strings.Contains(out, "input:") {
		t.Errorf("expected 'Input:' label in expanded view, got: %s", out)
	}
	if !strings.Contains(out, "Result:") && !strings.Contains(out, "result:") {
		t.Errorf("expected 'Result:' label in expanded view, got: %s", out)
	}
	// Verify bordered box structure
	if !strings.Contains(out, "╭") || !strings.Contains(out, "╰") {
		t.Errorf("expected bordered box in expanded view")
	}
}
